            ...
            if self.last_llm_response != llm_output.strip():
                try:
                    ...
                    audio = self.pipe.generate(
                        llm_output.strip(), step_callback=should_abort
                    )
                    inference_time = time.time() - start
                    logging.info(
                        f"[WhisperSpeech INFO:] TTS inference done in {inference_time} ms.\n\n"
                    )
            ...
            if self.last_llm_response != llm_output.strip():
                try:
                    ...
                    audio_response = self.openai_client.audio.speech.create(
                        model="tts-1",
                        voice="nova",
                        input=llm_output.strip(),
                    )
                    inference_time = time.time() - start
                    logging.info(
                        f"[OpenAI Speech INFO:] TTS inference done in {inference_time} ms.\n\n"
                    )
                    self.output_audio = audio_response.read()
                    self.last_llm_response = llm_output.strip()
                except TimeoutError:
                    pass
