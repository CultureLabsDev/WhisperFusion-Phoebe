...
from tts_service import OpenAISpeechTTS
...
def parse_arguments():
    ...
    parser.add_argument("--mistral", action="store_true", help="Mistral")
    parser.add_argument(
    ...
    parser.add_argument("--phi", action="store_true", help="Phi")
    parser.add_argument(
        "--phi_tensorrt_path",
        type=str,
    ...
    parser.add_argument("--gpt", action="store_true", help="GPT")
...
if __name__ == "__main__":
    ...
    if args.mistral:
        if not args.mistral_tensorrt_path or not args.mistral_tokenizer_path:
            raise ValueError(
                "Please provide mistral_tensorrt_path and mistral_tokenizer_path to run the pipeline."
            )
            import sys
    ...
    if args.phi:
        if not args.phi_tensorrt_path or not args.phi_tokenizer_path:
            raise ValueError(
                "Please provide phi_tensorrt_path and phi_tokenizer_path to run the pipeline."
            )
            import sys
    ...
    # audio process
    tts_runner = OpenAISpeechTTS()
    tts_process = multiprocessing.Process(
        target=tts_runner.run, args=("0.0.0.0", 8888, audio_queue)
    )
    tts_process.start()
